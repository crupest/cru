...
TEST_CASE("Mock UI application owns the thread singleton safely",
...

  {
    ...
    REQUIRE(app.GetPlatformId() == MockResource::kPlatformId);
  }

  REQUIRE(IUiApplication::GetInstance() == nullptr);
}

TEST_CASE("Mock UI application restores previous singleton",
          "[platform][gui][mock][app][singleton]") {
  MockUiApplication app;
...
TEST_CASE("UI application scoped registration temporarily overrides singleton",
          "[platform][gui][mock][app][singleton]") {
  MockUiApplication app;
  REQUIRE(IUiApplication::GetInstance() == &app);

  {
    IUiApplication::ScopedRegistration registration(nullptr);
    REQUIRE(IUiApplication::GetInstance() == nullptr);
  }
...
TEST_CASE("Mock UI application graphics factory ownership is explicit",
  ...
  {
    MockUiApplication app(new FakeGraphicsFactory(&owned_destruction_count),
                          true);
  ...
  {
    MockUiApplication app(&borrowed_graphics_factory, false);
    REQUIRE(app.GetGraphicsFactory() == &borrowed_graphics_factory);
  }
