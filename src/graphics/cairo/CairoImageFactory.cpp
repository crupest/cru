#include "cru/graphics/cairo/CairoImageFactory.h"
#include <cru/Exception.h>
#include "cru/Check.h"
#include "cru/graphics/cairo/CairoImage.h"
#include "cru/graphics/cairo/CairoResource.h"
...
std::unique_ptr<CairoImage> DecodePng(CairoGraphicsFactory* factory,
  ...
  png_set_read_fn(png_ptr, stream,
                  [](png_structp png_ptr, png_bytep data, png_size_t length) {
                    auto stream =
                        static_cast<Stream*>(png_get_io_ptr(png_ptr));
...
void EncodePng(cairo_surface_t* cairo_surface, Stream* stream) {
...
std::unique_ptr<IImage> CairoImageFactory::DecodeFromStream(
  ...
  if (IsPngHeader(buffer, 8)) {
    stream->Seek(0, Stream::SeekOrigin::Begin);
...
void CairoImageFactory::EncodeToStream(IImage* image, Stream* stream,
