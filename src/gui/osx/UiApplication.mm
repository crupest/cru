...
class OsxUiApplicationPrivate {
 ...
 private:
  ...
  std::unique_ptr<graphics::quartz::QuartzGraphicsFactory> quartz_graphics_factory_;
};
...
std::optional<String> OsxUiApplication::ShowSaveDialog(SaveDialogOptions options) {
  ...
  [panel setTitle:(NSString*)::cru::osx::ToCF(options.title).ref];
  [panel setPrompt:(NSString*)::cru::osx::ToCF(options.prompt).ref];
  [panel setMessage:(NSString*)::cru::osx::ToCF(options.message).ref];

  NSMutableArray* allowed_content_types = [NSMutableArray array];

  for (const auto& file_type : options.allowed_file_types) {
    [allowed_content_types
        addObject:[UTType typeWithFilenameExtension:(NSString*)::cru::osx::ToCF(file_type).ref]];
  }
  ...
  if (model_result == NSModalResponseOK) {
    return ::cru::osx::FromCF((CFStringRef)[[panel URL] path]);
  } else {
    return std::nullopt;
  }
}
...
std::optional<std::vector<String>> OsxUiApplication::ShowOpenDialog(OpenDialogOptions options) {
  ...
  if (model_result == NSModalResponseOK) {
    ...
    for (NSURL* url in [panel URLs]) {
      result.push_back(::cru::osx::FromCF((CFStringRef)[url path]));
    }
    return result;
  } else {
    return std::nullopt;
