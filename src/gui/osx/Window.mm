...
#include "cru/Check.h"
#include "cru/graphics/NullPainter.h"
#include "cru/graphics/quartz/Convert.h"
#include "cru/graphics/quartz/Painter.h"
#include "cru/gui/TimerHelper.h"
#include "cru/gui/osx/Cursor.h"
#include "cru/gui/osx/InputMethod.h"
...
using cru::graphics::quartz::Convert;
...
void OsxWindowPrivate::CreateWindow() {
  ...
  [window_ setTitle:(NSString*)::cru::osx::ToCF(title_).ref];
...
Rect OsxWindowPrivate::RetrieveContentRect() {
  ...
  return cru::graphics::quartz::Convert(rect);
}
...
void OsxWindow::SetTitle(String title) {
  ...
  if (p_->window_) {
    [p_->window_ setTitle:(NSString*)::cru::osx::ToCF(title).ref];
  }
}
...
std::unique_ptr<graphics::IPainter> OsxWindow::BeginPaint() {
  ...
  return std::make_unique<cru::graphics::quartz::QuartzCGContextPainter>(
...
- (instancetype)init:(cru::platform::gui::osx::details::OsxWindowPrivate*)p
  ...
  [super initWithFrame:cru::graphics::quartz::Convert(frame)];
  _p = p;
...
- (void)setMarkedText:(id)string
  ...
  composition_text.text = ::cru::osx::FromCF((CFStringRef)[_input_context_text string]);
