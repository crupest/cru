...
FontPropertyEditor::FontPropertyEditor() {
  font_size_input_ =
      Input<float>::Create("Font Size", new datamodel::NumberDataType<float>(),
                           new datamodel::NumberRangeDataValidator<float>(
                               0.0f, true, std::nullopt, false));
  ...
  right_container_.AddChild(font_size_input_->GetRootControl());
...
std::shared_ptr<platform::graphics::IFont> FontPropertyEditor::GetValue() {
  return platform::gui::IUiApplication::GetInstance()
      ...
      ->CreateFont(font_family_text_.GetText(),
                   font_size_input_->GetValue().value_or(16.0f));
}
